...
import type { GuardianContact, Review, ShopInfo } from '../../types'
  ...
  return (
    <div className="h-full overflow-y-auto p-5">
      ...
      <div className="flex flex-col gap-6">
          ...
          {profile.shopInfo && <ShopSection shop={profile.shopInfo} />}

          <PhotosSection uid={profile.uid} />

          {/* 보호자 연동은 일반 회원에게만 (사장님 계정에는 불필요) */}
          {!profile.shopInfo && (
...
// 소상공인 가게 정보 (프로모션 관리는 사장님 전용 홈의 '프로모션' 탭에서)
function ShopSection({ shop }: { shop: ShopInfo }) {
  return (
    <section className="flex flex-col gap-3">
